...
- Muted playback exposes a compact sound-activation button centered inside the header (beside the brand on mobile). Enabling sound removes it; the player retains a compact mute control. Existing muted-autoplay attempts and browser-blocked playback fallback remain in place.
