...
- Home uses the Arcade vinyl/cream/amber design; the Arcade/Encore selector is removed. `HomeMusic` adds a top-right artwork/title/artist player with previous/play-pause/next controls, shared volume, and looping imported preview audio (first song: DragonForce). Playback starts on a user gesture and stops on leaving home. A Canvas ring around the disc reacts to real Web Audio frequency data; animation stops while paused/hidden and becomes static for reduced-motion preferences. Player/context/animation resources are released on unmount. Mobile stacks the player under the header. Desktop/mobile playback, pause, song switching, and navigation were browser-checked.
...
- Catalog detail places larger artwork beside the title/artist and the preview control immediately below them on desktop, reducing vertical height. Metadata remains below; short desktop screens use two metadata columns. Larger labels/buttons/list text remain in place, and mobile stacks artwork above the title/artist/preview. Detail content scrolls independently above a separate play button; mobile uses normal page scrolling. This layout was checked at 1280×720, 1440×900, and 390×844.
