...
- Home shows current controls and configuration; Play opens a Clone Hero-inspired artist/title list with search, artwork, album/charter/genre/year/duration, actual difficulties, and arrow/Enter keyboard navigation. Escape returns home when the list is focused. Missing metadata uses a dash rather than invented values.
