...
- `src/game/menu-fade.ts` separates saved volume from transition gain. Home and catalog previews fade in over 450 ms and fade out before pause, mute (home), track replacement, or sample completion. Catalog retains one audio element so outgoing samples fade before changing sources. New actions cancel obsolete fade callbacks; unmount still stops audio immediately to prevent playback leaking into gameplay.
