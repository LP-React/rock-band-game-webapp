...
export function GameScreen({ song, difficulty, settings, onSettings, onExit, onFullscreen }: { song: Song; difficulty: Difficulty; settings: Settings; onSettings: (value: Settings) => void; onExit: () => void; onFullscreen: () => void }) {
  ...
  return <main className="game-shell">
    ...
    <footer className="game-footer"><div><button disabled={loading} onClick={restart}>Reiniciar</button><button disabled={!running} onClick={() => void engine.current?.togglePause()}>{paused ? 'Continuar · ESC' : 'Pausa · ESC'}</button></div><label>Volumen <input type="range" min="0" max="200" value={settings.volume} onChange={event => onSettings({ ...settings, volume: Number(event.target.value) })} />{settings.volume}%</label><span>Pista {settings.speed.toFixed(1)}×</span></footer>
  </main>
}
