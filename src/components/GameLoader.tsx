...
export function GameLoader({ song, difficulty, message = 'Cargando…', leaving = false, error, onRetry, onBack }: { song?: LoadingSong; difficulty?: Difficulty; message?: string; leaving?: boolean; error?: string; onRetry?: () => void; onBack?: () => void }) {
  useMenuKeyboard(event => { if (event.key === 'Escape' && onBack) { event.preventDefault(); onBack() } })
  return <div className="game-loader" data-leaving={leaving} style={themeStyle(song?.theme) as CSSProperties} aria-busy={!error}>
