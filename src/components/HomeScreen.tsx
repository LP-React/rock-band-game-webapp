...
export function HomeScreen({ settings, onPlay, onConfig }: { settings: Settings; onPlay: () => void; onConfig: () => void }) {
  ...
  function closeModes() { setModes(false); playButton.current?.focus() }
  return <main className="menu-screen home-screen" data-variant="arcade">
    ...
    <section className="home-arena" aria-label="Menú principal">
      ...
      <nav className={`home-navigation ${modes ? 'choosing-mode' : ''}`} aria-label="Opciones del juego">
        <div className="home-main-actions"><button ref={playButton} className="menu-ribbon play-ribbon" aria-expanded={modes} aria-controls="play-modes" onClick={() => setModes(value => !value)} autoFocus><strong>Jugar</strong><b aria-hidden="true">↗</b></button><button className="menu-ribbon config-ribbon" onClick={onConfig}><strong>Configuración</strong><b aria-hidden="true">⚙</b></button></div>
