...
export function HomeMusic({ volume, canvas, index, onSongChange }: { volume: number; canvas: RefObject<HTMLCanvasElement | null>; index: number; onSongChange: (index: number) => void }) {
  ...
  return <div className="home-music" aria-label="Reproductor del menú">
    ...
    {muted && <button className="home-sound-prompt" aria-label="Activar sonido" onClick={() => void play(true)}><span aria-hidden="true">♫</span><span><strong>Activar sonido</strong><small>{playing ? 'La música está sonando en silencio' : 'Escucha la música del menú'}</small></span></button>}
